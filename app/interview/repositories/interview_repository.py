...
class InterviewRepository:
    async def get_candidate_interviews(
        ...
        with database.create_session() as db:
            query = db.query(models.Interview).filter_by(candidate_id=candidate_id)
            ...
            return {"data": query, "total_pages": total_pages}

    async def get_customer_interviews(
        self, customer_id: int, page: int, page_size: int
    ):
        with database.create_session() as db:
            query = db.query(models.Interview).filter_by(customer_id=customer_id)
